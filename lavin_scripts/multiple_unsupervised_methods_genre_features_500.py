...
ms.fit(data)
save_labels(ms, "meanshift_all_no_bin_seeding_genre_features_500.csv", df)

bandwidth2 = estimate_bandwidth(data, quantile=0.2, n_samples=400)
ms2 = MeanShift(bandwidth=bandwidth2, bin_seeding=True)
ms2.fit(data)
save_labels(ms2, "meanshift_all_w_bin_seeding_genre_features_500.csv", df)

# affinity all
af = AffinityPropagation().fit(data)
save_labels(af, "affinity_all_genre_features_500.csv", df)

# mini batch k-means all
mbkm = MiniBatchKMeans().fit(data)
save_labels(mbkm, "mini_batch_km_all_genre_features_500.csv", df)

# hierarchical all
wc = AgglomerativeClustering(linkage="ward", n_clusters=10).fit(data)
save_labels(wc, "ward_hierarchical_all_genre_features_500.csv", df)

wc2 = AgglomerativeClustering(linkage="average", n_clusters=10).fit(data)
save_labels(wc2, "avg_hierarchical_all_genre_features_500.csv", df)

wc3 = AgglomerativeClustering(linkage="complete", n_clusters=10).fit(data)
save_labels(wc3, "complete_hierarchical_all_genre_features_500.csv", df)
...
ms.fit(horror_data)
save_labels(ms, "meanshift_horror_no_seeding_genre_features_500.csv", df_horror)

# affinity just horror
af = AffinityPropagation().fit(horror_data)
save_labels(af, "affinity_horror_genre_features_500.csv", df_horror)

# mini batch k-means horror
mbkm = MiniBatchKMeans().fit(horror_data)
save_labels(mbkm, "mini_batch_km_horror_genre_features_500.csv", df_horror)

# hierarchical horror
wc = AgglomerativeClustering(linkage="ward", n_clusters=5).fit(horror_data)
save_labels(wc, "ward_hierarchical_horror_genre_features_500.csv", df_horror)

wc2 = AgglomerativeClustering(linkage="average", n_clusters=5).fit(horror_data)
save_labels(wc2, "avg_hierarchical_horror_genre_features_500.csv", df_horror)

wc3 = AgglomerativeClustering(linkage="complete", n_clusters=5).fit(horror_data)
save_labels(wc3, "complete_hierarchical_horror_genre_features_500.csv", df_horror)
